...
-- Public read on phones; authenticated insert/update
...
-- Drafts only by user (tg_id is not auth.uid() so keep restricted to service role)
...
-- Reports and subscriptions via service role from the bot
...
-- Events for metrics (active users, errors, etc.)
